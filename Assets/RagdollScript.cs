    ...
    void FixedUpdate()
    {
    	if(true)
    	{
            ...
            if(Physics.Raycast(head.transform.position, -GalaxyManager.getGravityVector(head), out rch, 1f, LayerMask.GetMask("planet_object")))
            {
                ...
                if(Vector3.Distance(rch.point, IK[2].GetComponent<DitzelGames.FastIK.FastIKFabric>().Target.position) > 0.12f)
                {
                	Debug.Log("step r");
                ...
                else if(Vector3.Distance(rch.point, IK[3].GetComponent<DitzelGames.FastIK.FastIKFabric>().Target.position) > 0.12f)
                {
                	Debug.Log("step l");
